...
void PieChart::draw(QPainter &painter)
{
    ...
    // 绘制扇形
    double startAngle = 90.0 * 16;  // Qt 从 3 点钟方向起算，-90° 调到 12 点
    painter.save();
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // 百分比标签（在扇形中间角度显示）
        if (ratio >= 0.03) {  // 太小的扇形不显示标签
            double midAngleDeg = (startAngle + spanAngle / 2.0) / 16.0;
            double rad = qDegreesToRadians(midAngleDeg - 90.0);
            double labelR = r * 0.62;
            double tx = cx + labelR * cos(rad);
            double ty = cy + labelR * sin(rad);

            painter.setPen(Qt::white);
            QFont pctFont = painter.font();
            pctFont.setPointSize(10);
            pctFont.setBold(true);
            painter.setFont(pctFont);
            QRectF pctRect(tx - 30, ty - 10, 60, 20);
            painter.drawText(pctRect, Qt::AlignCenter,
                             QString::number(ratio * 100, 'f', 1) + "%");
        }
